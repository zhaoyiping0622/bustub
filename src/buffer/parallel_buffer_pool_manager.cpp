...
ParallelBufferPoolManager::ParallelBufferPoolManager(size_t num_instances, size_t pool_size, DiskManager *disk_manager,
  ...
  buffer_pool_manager_instance_ = new BufferPoolManagerInstance *[num_instances];
...
Page *ParallelBufferPoolManager::NewPgImp(page_id_t *page_id) {
  ...
  // is called
  for (size_t i = 0; i < num_instances_; i++) {
    auto current = instance_index_.fetch_add(1) % num_instances_;
    auto page = buffer_pool_manager_instance_[current]->NewPage(page_id);
    if (page != nullptr) {
      return page;
    }
  }
...
void ParallelBufferPoolManager::FlushAllPgsImp() {
  ...
  for (size_t i = 0; i < num_instances_; i++) {
    buffer_pool_manager_instance_[i]->FlushAllPages();
  }
}
